...
export default function ActivityTimerScreen({ navigation }) {
  ...
  const history = activityState.history;
  const dispatch = useDispatch();
  const [selected, setSelected] = useState(activities[0]);
  const [running, setRunning] = useState(false);
  const [seconds, setSeconds] = useState(0);
  const [completed, setCompleted] = useState(null);
  const saving = !!activityState.pendingSession;
  const elapsed = useRef({ base: 0, startedAt: null });
  const [pulse] = useState(() => new Animated.Value(0));

  useEffect(() => {
    if (!running) return undefined;
    const id = setInterval(() => setSeconds(elapsed.current.base + Math.floor((Date.now() - elapsed.current.startedAt) / 1000)), 250);
  ...
  const toggle = () => {
    if (saving) return;
    setCompleted(null);
    if (running) { elapsed.current.base = seconds; elapsed.current.startedAt = null; }
    else { elapsed.current.base = seconds; elapsed.current.startedAt = Date.now(); }
    setRunning((value) => !value);
    Haptics.selectionAsync().catch(() => {});
  };

  const reset = () => { if (saving) return; elapsed.current = { base: 0, startedAt: null }; setRunning(false); setSeconds(0); setCompleted(null); };

  const finish = async () => {
    if (!seconds || saving || completed) return;
    const durationSeconds = running ? elapsed.current.base + Math.floor((Date.now() - elapsed.current.startedAt) / 1000) : seconds;
    const session = { id: Date.now(), activity: selected.label, durationSeconds, startedAt: new Date().toISOString(), minutes: Math.floor(durationSeconds / 60), calories: Math.round(durationSeconds / 60 * selected.rate), when: `Today · ${new Date().toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}` };
    setRunning(false);
    setCompleted(session); setSeconds(0); elapsed.current = { base: 0, startedAt: null };
    dispatch(completeActivity(session)).then((result) => {
      if (completeActivity.fulfilled.match(result)) Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch(() => {});
  ...
  return (
    <Screen contentStyle={styles.page}>
      ...
      <SyncFeedback label="Activity" pending={saving} error={activityState.error} onRetry={() => activityState.failedSession ? confirmRetry('Activity', () => dispatch(completeActivity(activityState.failedSession))) : dispatch(loadActivities())} />
