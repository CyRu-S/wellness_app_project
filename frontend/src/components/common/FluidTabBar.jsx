...
function FluidTabItem({
  ...
  const [slotWeight] = useState(() => new Animated.Value(targetWeight));
  useEffect(() => {
    ...
    const transition = Animated.parallel([
      Animated.timing(progress, { toValue: focused ? 1 : 0, duration, easing, useNativeDriver: false }),
      Animated.timing(slotWeight, { toValue: targetWeight, duration, easing, useNativeDriver: false }),
    ], { stopTogether: false });
    transition.start();
    return () => transition.stop();
  }, [focused, progress, reduceMotion, slotWeight, targetWeight]);
  const onPress = () => {
