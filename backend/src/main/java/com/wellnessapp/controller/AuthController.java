...
@RestController @RequestMapping("/api/auth") @RequiredArgsConstructor public class AuthController {
    ...
    private final com.wellnessapp.service.EmailVerificationService emailVerification;
    ...
    @PostMapping("/verify-email") MessageResponse verifyEmail(@Valid @RequestBody VerifyEmailRequest request) { return emailVerification.verify(request); }
    @PostMapping("/resend-verification") MessageResponse resendVerification(@Valid @RequestBody ForgotPasswordRequest request) { return emailVerification.resend(request); }
}
